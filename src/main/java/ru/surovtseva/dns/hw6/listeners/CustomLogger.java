package ru.surovtseva.dns.hw6.listeners;
...
import ru.surovtseva.dns.hw6.helpers.ScreenshotMaker;
...
public class CustomLogger implements WebDriverEventListener {
    ...
    public void beforeClickOn(WebElement webElement, WebDriver webDriver) {
        String log = String.format("Before click on element with tagname = %s, enabled = %s, is_displayed = %s",
                webElement.getTagName(), webElement.isEnabled(), webElement.isDisplayed());
        logger.info(log);
    }

    @Override
    public void onException(Throwable throwable, WebDriver webDriver) {
        webDriver.manage().logs().get(LogType.BROWSER).getAll();
        String fileName = "failure-screenshot-" + System.currentTimeMillis() + ".png";
        ScreenshotMaker.makeScreenshot(webDriver, fileName);
